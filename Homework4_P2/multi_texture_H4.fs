...
uniform sampler2D texture_3; //texture3
...
void main(void)                                                   
{
	...
	vec4 tex_color_3 = texture(texture_3, pass_TexCoord);
    ...
    if(texture_blend == 0)
    {
        color = 0.1 * pass_Color + tex_color + 0.2*tex_color_light + 0.2*tex_color_3 ;
    }
    else if(texture_blend == 1)
    {
        color = tex_color * tex_color_light * tex_color_3;
    }
    else if(texture_blend == 2)
    {
        color = (1-tex_color_light)*(1-tex_color)*(1-tex_color_3) ;
    }
    else
    {
        color = 0.1 * pass_Color + tex_color + tex_color_light + tex_color_3;
    }
