...
void CameraManipulator::init_camera_param(void)
{
    ...
    _cameraRotationAxis = glm::vec3(0.0,0.0,1.0);
    _currentAngle = 0.0;
	_rolling_speed = 0.05f;
    _traveling_speed = 10000.0f;
    _camera_distance = 50.0f;
    
    _lastframePosDistance = glm::vec3(0.0,0.0,0.0);
}

bool CameraManipulator::keystrokes_initc(int key_stroke)
{
...
glm::vec3 CameraManipulator::toWindowCoord( double x, double y )
{
    ...
    coord.x =  (x - _window_width ) / _window_width;
    coord.y = -(y - _window_height) / _window_height;
