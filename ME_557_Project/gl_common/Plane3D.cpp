...
void GLPlane3D::initVBO(void)
{
    ...
    float* vertices = new float[_num_vertices * 6];
    ...
    for(int i=0; i<_vertex_points.size() ; i++)
    {
        ...
        vertices[(i*6)] = v.x(); vertices[(i*6)+1] = v.y(); vertices[(i*6)+2] = v.z();
		vertices[(i * 6) + 3] = v.u(); vertices[(i * 6) + 4] = v.v(); vertices[(i * 6) + 5] = v.w();
    ...
    glBufferData(GL_ARRAY_BUFFER, _num_vertices * 6 * sizeof(GLfloat), vertices, GL_STATIC_DRAW); // Set the size and data of our VBO and set it to STATIC_DRAW
    
    int locPos = glGetAttribLocation(_program, "in_Position");
    glVertexAttribPointer((GLuint)locPos, 3, GL_FLOAT, GL_FALSE, 6*sizeof(GLfloat), 0); // Set up our vertex attributes pointer
    glEnableVertexAttribArray(locPos); //
    
    
    int tex_idx = glGetAttribLocation(_program, "in_TexCoord");
    glVertexAttribPointer((GLuint)tex_idx, 3, GL_FLOAT, GL_TRUE, 6*sizeof(GLfloat),
